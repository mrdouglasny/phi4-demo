...
if method=="sum":
    ...
    def fix_signs_in_vector(v,tol=1e-6):
        d=np.arange(1.0,v.size+1,1.0)**(-1)
        s=(v*d).sum()
        factor=np.conjugate(phase_factor(s))
        v*=factor
        return factor
...
class AbelianTensor(TensorCommon):
    ...
    ###### Added by Kolia block
    ###### Routines for fixing the positive signs in abelian matricies
    def fix_signs_of_columns_in_sect(self,sect_sig,tol=1e-9):
        def fsiv(v):
            return fix_signs_in_vector(v,tol=tol)
        num_of_vectors=np.shape(self.sects[sect_sig])[-1]
        signs=np.apply_along_axis(fsiv,0,self.sects[sect_sig].reshape((-1,num_of_vectors)))   
        return signs   
        
    def fix_signs_of_columns(self,tol=1e-9):
        # note that even we run through all the blocks, for the part of the matrix with fixed second qnum, 
        # for example (q,1) there is only one nonzero block if charge=0 and inv=true as, depending on the directions, 1-q=0 
        # or 1+q=0 fix q=-1. (in Z2 case q=1).

        if self.invar!=True:
            warnings.warn("fix_signs_of_columns matrix is not invariant")

        sections_for_signs=dict()    
        
        for sect_sig in self.sects:
            sections_for_signs[(sect_sig[-1],)]=self.fix_signs_of_columns_in_sect(sect_sig,tol=tol)
        ...
        # Set the blocks of signs
        for k, v in sections_for_signs.items():
            signs[k] = v
    ...
    def matrix_eig(
        ...
        U.fix_signs_of_columns(tol=1e-10) # added by Kolia
    ...
    def matrix_svd(
        ...
        signs = U.fix_signs_of_columns(tol=1e-10);  # added by Kolia
