...
using PyCall
# Add src/GiltTNR to Python path (works whether we're in root or src/)
pushfirst!(pyimport("sys")."path", joinpath(@__DIR__, "GiltTNR"))
@pyinclude(joinpath(@__DIR__, "GiltTNR/GiltTNR2D_essentials.py"))
...
# using CairoMakie  # Commented out due to Julia 1.12 incompatibility
...
# import CairoMakie: lines, scatter  # Commented out due to Julia 1.12 incompatibility
...
# spec = deserialize("IsingExactLevels")  # File not found - commented out
...
# spec = deserialize("IsingEvenExactLevels")  # File not found - commented out
